...
//			System.out.println("not match!");
	...
	public static String hssfCellTypeChkToString(Cell cell) {
		String str = null;
		if(cell == null ) {
			return str;
		}
	...
	public static String xssfCellTypeChkToString(Cell cell) {
		String str = null;
		if(cell == null ) {
			return str;
		}
		switch( cell.getCellType()) {
	        ...
	        case XSSFCell.CELL_TYPE_NUMERIC :
	        	//boolean date = StringUtil.isLastChkDttm(cell.toString());
	            if( org.apache.poi.ss.usermodel.DateUtil.isCellDateFormatted(cell)){
    ...
    private static final Pattern DATE_PATTERN = Pattern.compile("^((19|20)[0-9]{2}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[0-1]))$");
    public static boolean isLastChkDttm(final String input) {
    	return DATE_PATTERN.matcher(input).matches();
	}
	public static void main(String [] args) throws ParseException{
		/*TreeSet<TreeVO> treeId = new TreeSet(new chkDupID());
		
		treeId.add(new TreeVO("1234"));
		treeId.add(new TreeVO("3421"));
		treeId.add(new TreeVO("1333"));
		treeId.add(new TreeVO("1234"));
		treeId.add(new TreeVO("1432"));
		treeId.add(new TreeVO("1222"));
		treeId.add(new TreeVO("1243"));
		
		for(Iterator<TreeVO> it = treeId.iterator(); it.hasNext();) {
		    TreeVO t1 = (TreeVO)it.next();
		    System.out.println("DEVICE_ID==["+t1.getCHECKID()+"]");
	    }*/
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date d = (Date) sdf.parse("16-8월-2015");
	    String str = sdf.format(d);
		System.out.println("surffix["+str);
	}
	...
	// encoding : UTF-8, EUC-KR
		...
		public static int getStringCardSize(String param) {
			
			if ( isEmpty( param ) ) return 0;
			
			// LENGTH(TRIM(REPLACE(#{paymentConfmNoSeacrhKey},'*','')))
			
			return param.replace("*", "").trim().length();
		}
}
