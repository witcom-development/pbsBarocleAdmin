	...
	<div class="allwrap">
									...
									<tbody>
										...
										<tr>
											<th>구분
												...
												<td>
													<select class="select-category01" id="comCd" name="comCd">
													<option value="">선택</option>
													<c:forEach items="${gubun }" var="gubun">
														<option value="${gubun.comCd }" <c:if test="${gubun.comCd eq opinionBoardView.cateCD}">selected="true"</c:if>>${gubun.cdDesc1 }</option>
													</c:forEach>
												</select>
											</td>
										</tr>
										...
										<tr>
											<th>등록일시</th>
											<td>
												<%-- <fmt:formatDate value="${opinionBoardView.usr_regDttm}" pattern="yyyy-MM-dd HH:mm:ss"/> --%>
												<fmt:parseDate var="usr_regDttm" value="${opinionBoardView.usr_regDttm}" pattern="yyyy-MM-ddHH:mm:ss" />
												<fmt:formatDate value="${usr_regDttm}" pattern="yyyy-MM-dd HH:mm:ss"/>
											</td>
										</tr>
										<tr>
											<th><span>내용</span></th>
											<td>
												...
												<c:choose>
												<c:when test="${opinionBoardView.qestnHTMLContent != null}">
													<c:out value="${opinionBoardView.qestnHTMLContent}" escapeXml="false" />
												</c:when>
												<c:otherwise>
													<c:out value="${fn:replace(opinionBoardView.qestnTextContent,LF,'<br/>')}" escapeXml="false" />
												</c:otherwise>
												</c:choose>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<c:forEach items="${daumEditorImageList}" var="daumEditorVO" varStatus="status">
													<a href="#" onclick="popImg('<c:out value='${daumEditorVO.imgURL}'/>');return false;"><c:out value="${daumEditorVO.imgName}"/></a>
												</c:forEach>
											</td>
										</tr>
										</c:if>
										...
										<tr>
											<th>답변일시</th>
											<td>
												<%-- <fmt:formatDate value="${opinionBoardView.modDttm}" pattern="yyyy-MM-dd HH:mm:ss"/> --%>
												<fmt:parseDate var="modDttm" value="${opinionBoardView.modDttm}" pattern="yyyy-MM-ddHH:mm:ss" />
		...
		<script type="text/javascript">		
			...
			function popImg(imgUrl){
				window.open("${homeURL}"+imgUrl, "_blank", "이미지팝업");
			}
