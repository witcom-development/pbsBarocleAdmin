...
<script type="text/javascript">
	<%-- 현재 메뉴의 값을 절대적으로 표시하야 함. --%> var currentPageName = "faq";
</script>
</head>
<body>
	<div class="allwrap">
				...
				<div class="content">
					...
					<div class="page">
						<form id="tx_editor_form" name="tx_editor_form" method="post" accept-charset="utf-8" onsubmit="return false;">
							<input type="hidden" id="mode" name="mode" value="${faqView.mode}" />
							<input type="hidden" id="faqSeq" name="faqSeq" value="${faqView.faqSeq}" />
							<input type="hidden" id="tempCateCD" name="tempCateCD" value="${faqView.cateCD}" />
							<fieldset>
								<legend>FAQ작성</legend>
									...
									<tbody>
										<tr>
											...
											<td class="top">
												...
												<a href="/admin/customer/faq/getCategoryPopUp.do" class="winNewPop modal btnType02 thid"  title="카테고리 관리" data-pmt="" data-width="440" data-height="455">카테고리 관리</a>
											</td>
										</tr>
										<tr>
											<th>표시순서</th>
											<td>
												<span id="cateCDField"></span> : <span id="totalCount"></span>개 중 <span class="input-text03"><input id="qestnOrd" name="qestnOrd" type="text" maxlength="4" numberOnly="true" value="${faqView.qestnOrd}" /></span>번째
											</td>
										</tr>
										<tr>
											<th>제목</th>
											<td>
												<span class="input-text04"><input id="qestnTitle" name="qestnTitle" type="text" value="${faqView.qestnTitle}"/></span>
											</td>
										</tr>
										<tr class="editorLine">
											<th><span>내용</span></th>
											<td class="editor">
												...
												<div style="display:none;"><textarea id="answer_HTML_content" path="content"></textarea></div>
												<!--daum editor E-->
											</td>
										</tr>
									</tbody>
								...
								<div class="btnArea">
									<span class="left"><button class="btnType01 bc_gray" onclick="location.href='/admin/customer/faq/faqList.do?currentPageNo=<c:out value="${faqView.currentPageNo}"/><c:out value="${parameterInfo}" escapeXml="false"/>';">목록</button></span>
									<span class="right">
										<c:if test="${faqView.faqSeq>0}">
											<button class="btnType01" onclick="deleteContent('<c:out value="${faqView.faqSeq}"/>','<c:out value="${faqView.cateCD}"/>');">삭제</button>
										</c:if>
		...
		<script type="text/javascript">
			...
			function deleteContent(faqSeq,cateCD){
				if(confirm("삭제 하시겠습니까?")){
					location.href = "/admin/customer/faq/faqDelete.do?mode=delete&faqSeq="+faqSeq+"&tempCateCD="+cateCD;
				} else return false;
			}
			...
			function validForm(editor) {
				...
				if(mode=='insert'){
					$("#tx_editor_form").attr('action','/admin/customer/faq/faqInsert.do').submit();
				} else if(mode=='update'){
					$("#tx_editor_form").attr('action','/admin/customer/faq/faqUpdate.do').submit();
				} else {
		...
		<textarea id="answer_TEXT_content" style="display:none;"><c:out value="${faqView.answerHTMLContent}"/></textarea>
		<script type="text/javascript">
			$(document).on("keyup", "input:text[numberOnly]", function() {$(this).val( $(this).val().replace(/[^0-9]/gi,"") );});
			var faqInit = {};
			faqInit.getCommonCode = function(){
				$("#cateCDField").text($("#cateCD").find("option:selected").text());
				$.ajax({
					url : "/admin/customer/faq/faqCategoryInfo.do",
					type : "get",
					data : { "cateCD" : $("#cateCD").find("option:selected").val()},
					contentType: "charset=utf-8",
					success : function(data){
						$("#totalCount").text(data.totalCount);
						var nextCount = data.nextCount;
						if($("#tempCateCD").val()==$("#cateCD").val()){
							$("#qestnOrd").val("<c:out value='${faqView.qestnOrd}'/>");
						} else {
							$("#qestnOrd").val(++nextCount);
						}
					},
					error : function(){
			...
			$(document).ready(function(){
				...
				/* 표시순서 카테고리 변경 */
				faqInit.getCommonCode();
				$("#cateCD").on('change',function(){
					faqInit.getCommonCode();
				});
				/* 이미지 */
				var attachments = {};
