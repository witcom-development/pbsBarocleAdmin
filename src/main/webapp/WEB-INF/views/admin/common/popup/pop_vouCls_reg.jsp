...
<body>
	...
	<div class="cntbox">
	<form id="frmVou">
		<input type="hidden" name="payment_seq" value="${payment_seq}">
...
<script type="text/javascript">
	...
	regVoucherInfo = function() {
		...
		if($("input[name=payment_cls_cd]").val() == ""){
			alert("이용권 구분을 선택해주세요.")
			return;
		}

		
		commonAjax.postAjax("/admin/service/member/regVoucherClsAjax.do", "json", 
   				$("#frmVou").serialize(), 
   				function(data) {
	   				if(data) {
						if(data.isCreate == 1) {
							alert("이용권 구분이 등록되었습니다.");
							opener.reloadPage();
							window.close();
						} else{
							alert("이용권 구분 등록에 실패하였습니다.");
							window.close();
						}
					}	
		});
