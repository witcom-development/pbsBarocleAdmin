...
<script type="text/javascript">
	...
	$("#dispchDiv #dtlTimeSt").on("change",function(){//분배 외 시간 선택박스
    ...
    workList.ajax = function(data){
    	...
    	if(data == "mainWork"){//당일날 근태 입력 했다면 화면에 그 값을 보여줌.
	    	...
	    	$.ajax({
				...
				success: function(data){
					...
					if(!workVO.workTime){
						...
						$("#dispchDiv [name='etcBikeSeq']").val( nvl(workVO.etcBikeSeq, "") );
    	...
    	}else if(data == "getCd"){//근태 입력 분배 외 사유 selectbox 데이터 불러오기.
        	...
        	if(!regExp.test(dtlTime)) {
        		...
        		alert("분배 외 시간에 숫자만 입력하세요.");
        		dr = false;
        		return true;
        	}
        	
        	
        	var etcBikeSeq  =  $("#dispchDiv [name='etcBikeSeq']").val();
        	var moveBikeCnt =  $("#dispchDiv [name='moveBikeCnt']").val();
        	var moveBikeCd  =  nvl( $("#dispchDiv select[name='moveBikeCd']").val(), "");
        	var moveEtc     =  nvl( $("#dispchDiv [name='moveEtc']").val()   , "");
        	
			if(!regExp.test( moveBikeCnt )) {
        		
				$("#dispchDiv [name='moveBikeCnt']").focus();
        		alert("입고건수에 숫자만 입력하세요.");
			...
			if ( moveBikeCnt > 0 ){
				 if ( moveBikeCd == ""  ) {
					 $("#dispchDiv [name='moveBikeCd']").focus();
					alert("입고사유를 선택해주세요.");
	        		dr = false;
	        		return true;
				 }
				
				 if ( moveBikeCd == "9" && moveEtc == ""  ) {
					 $("#dispchDiv [name='moveEtc']").focus();
					alert("기타 일 경우 상세사유를 입력해주세요.");
	        		dr = false;
	        		return true;
				 }
			...
			var relocateBikeCnt =  nvl( $("#dispchDiv [name='relocateBikeCnt']").val(), "0");
			var relocateBikeCd  =  nvl( $("#dispchDiv select[name='relocateBikeCd']").val() , "");
			var relocateEtc     =  nvl( $("#dispchDiv [name='relocateEtc']").val()    , "");
			if(!regExp.test( relocateBikeCnt )) {
        		
        		alert("출고건수에 숫자만 입력하세요.");
        		dr = false;
        		return true;
        	}
			
			if ( relocateBikeCnt > 0 ){
				
				 if ( relocateBikeCd == "" ) {
					 $("#dispchDiv [name='relocateBikeCd']").focus();
					alert("출고사유를 선택해주세요.");
	        		dr = false;
	        		return true;
				 }
				
				 if ( relocateBikeCd == "9" &&  relocateEtc == ""  ) {
					 $("#dispchDiv [name='relocateEtc']").focus();
					alert("기타 일 경우 상세사유를 입력해주세요.");
	        		dr = false;
	        		return true;
				 }
    		...
    		if(dr){//문제없을 때만 저장 처리
    			...
    			params.etc = etc;
    			params.moveBikeCnt = nvl(moveBikeCnt,"0");
    			params.moveBikeCd = moveBikeCd;
    			params.moveEtc = moveEtc;
    			params.relocateBikeCnt = nvl( relocateBikeCnt, "0");
    			params.relocateBikeCd = relocateBikeCd;
    			params.relocateEtc = relocateEtc;
    			params.etcBikeSeq = etcBikeSeq;
	...
	dispatch.ajax = function(data){
		...
		if(data == "mainVcle") {//당일날 차량 등록 했다면 화면에 그 값을 보여줌.
	    	...
	    	$.ajax({
				...
				success: function(data){
					...
					if( vcleVO.vcleSeq != null && vcleVO.vcleSeq != 0 && vcleVO.vcleSeq != '') {
						...
						$("#dispchDiv [name='notice']").val(vcleVO.notice);
						if( vcleVO.driverType1 ) $("#dispchDiv [name='driverType1']").val(vcleVO.driverType1);
						if( vcleVO.driverType2 ) $("#dispchDiv [name='driverType2']").val(vcleVO.driverType2);						
						...
						$("#dispchDiv [name='vcleNo1']").val("");
						$("#dispchDiv [name='vcleNo2']").val("");
						
						$("#dispchDiv [name='startTime']").val("");
						$("#dispchDiv [name='startTime1']").val("");
						$("#dispchDiv [name='startTime2']").val("");
						
						$("#dispchDiv [name='endTime']").val("");
						$("#dispchDiv [name='endTime1']").val("");
						$("[name='endTime2']").val("");
						
						$("#dispchDiv [name='bfMileage']").val("");
						$("#dispchDiv [name='bfMileage1']").val("");
						$("#dispchDiv [name='bfMileage2']").val("");
						
						$("#dispchDiv [name='afMileage']").val("");
						$("#dispchDiv [name='afMileage1']").val("");
						$("#dispchDiv [name='afMileage2']").val("");
						
						$("#dispchDiv [name='driverType']").val("");
						$("#dispchDiv [name='driverType1']").val("");
						$("#dispchDiv [name='driverType2']").val("");
 						
						$("#dispchDiv [name='notice']").val(vcleVO.notice);
						$("#dispchDiv [name='notice1']").val("");
						$("#dispchDiv [name='notice2']").val("");
	    	...
	    	var paramList = {
	    			...
	    			// 20220120 삭제구분자 추가
	    			delYN1		: null,		delYN2		: null,		delYN		: null,
	    			// 20220120 알림내역 추가
	    			notice1		: null,		notice2		: null,		notice		: null
	    		};
	  		...
	  		// 한글이 깨지는 문제로 직접 
	  		paramList["notice"]  = $("#dispchForm [name='notice']").val();
	  		paramList["notice1"] = $("#dispchForm [name='notice1']").val();
	  		paramList["notice2"] = $("#dispchForm [name='notice2']").val();
	...
	function fn_save_befor_validate(paramList) {
		...
		if( paramList.vcleNo != null && paramList.vcleNo !="" ) {
	    	...
	    	if( !isValidation( "#dispchDiv #startTime"  , paramList.startTime , "운행시작 시간을 선택하세요."    , "null"   )) return false;
	    	
	    	if( !isValidation( "#dispchDiv #endTime"  , paramList.endTime , "운행종료 시간을 선택하세요."    , "null"   )) return false;
    	...
    	if( paramList.vcleNo1 != null && paramList.vcleNo1 !="" ) {
    		...
    		if( !isValidation( "#dispchDiv #endTime1"  , paramList.endTime1 , "운행종료 시간을 선택하세요."    , "null"   )) return false;
    	...
    	if( paramList.vcleNo2 != null && paramList.vcleNo2 !="" ) {
    		...
    		if( !isValidation( "#dispchDiv #endTime2"  , paramList.endTime2 , "운행종료 시간을 선택하세요."    , "null"   )) return false;
		...
		if( $(this).val() != '' ) {
			for(var i =0; i< dispatch.vcleInfoList.length; i++){
				if(dispatch.vcleInfoList[i].vcleNoSeq ===  $(this).val() ){
					...
					if( !isEmpty( dispatch.vcleInfoList[i].notice) ) {
						$("#dispchDiv [name='notice']").val( dispatch.vcleInfoList[i].notice );
						alert(" 이전 차량 전달사항 : " + dispatch.vcleInfoList[i].notice )
					}
				}
			}
		} else {
			$("#dispchDiv [name='bfMileage']").val( "" );
			$("#dispchDiv [name='notice']").val( "");
		...
		if( $(this).val() != '' ) {
			for(var i =0; i< dispatch.vcleInfoList.length; i++){
				if(dispatch.vcleInfoList[i].vcleNoSeq ===  $(this).val() ){
					...
					if( !isEmpty( dispatch.vcleInfoList[i].notice) ) {
						$("#dispchDiv [name='notice1']").val( dispatch.vcleInfoList[i].notice );
						alert(" 이전 차량 전달사항 : " + dispatch.vcleInfoList[i].notice )
					}
				}
			}
		...
		if( $(this).val() != '' ) {
			for(var i =0; i< dispatch.vcleInfoList.length; i++){
				if(dispatch.vcleInfoList[i].vcleNoSeq ===  $(this).val() ){
					...
					if( !isEmpty( dispatch.vcleInfoList[i].notice) ) {
						$("#dispchDiv [name='notice2']").val( dispatch.vcleInfoList[i].notice );
						alert(" 이전 차량 전달사항 : " + dispatch.vcleInfoList[i].notice )
					}
				}
			}
