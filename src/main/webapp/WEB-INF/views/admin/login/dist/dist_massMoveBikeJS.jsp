			...
			, function(data){
				...
				if ( data.list != null  ){
					...
					for(key in data.list){
						html += '<tr>';
						html += '<td>'+nvl(data.list[key].workFrHour,'')+'~'+nvl(data.list[key].workToHour,'')+'</td>';
						html += '<td>'+nvl(data.list[key].workDtl,'')+  '</td>';
						if( data.list[key].workDtlSeq != null && nvl(data.list[key].workDtlSeq,'') != "" ) {
							html += '<td>'+'<a href=\"javascript:coworkCancle(' +  data.list[key].coworkInfoSeq +"," +  data.list[key].workDtlSeq  + ')\">취소</a>' +'</td>';
						} else {
							html += '<td>'+'<a href=\"javascript:coworkSave(' + data.list[key].coworkInfoSeq + ')\">참여</a>' +'</td>';
						}
						html += '</tr>';
					}
				}
				  $("#coWorkDiv #coWorkDynamicTbody").empty();
				  $("#coWorkDiv #coWorkDynamicTbody").append(html);
				...
				if ( data.list != null && data.list.coworkInfoSeq > 0 ){
					...
					switch ( data.usrGrpCd ){
						...
						case "13" :	// 운영팀
						case "14" :	// 관리팀
							$("#coWorkFormDiv #btnCoworkSave").html("저장");					// 등록
							setDisplay( "#coWorkFormDiv #btnCoworkSave"     	, "show");	// 메인 등록, 수정
						break;
					}
					setDisplay( "#coWorkFormDiv #btnWrkerAdd"     		, "hide");
...
function coworkSave(coworkInfoSeq){
	
	$("#coWorkFormDiv input[name='coworkInfoSeq']").val(coworkInfoSeq);
...
function coworkCancle(coworkInfoSeq, coworkDtlSeq){

    if ( !confirm("정말 취소하시겠습니까?")) return;
    
    $("#coWorkFormDiv input[name='coworkInfoSeq']").val(coworkInfoSeq);
    $("#coWorkFormDiv input[name='coworkDtlSeq']").val(coworkDtlSeq);
