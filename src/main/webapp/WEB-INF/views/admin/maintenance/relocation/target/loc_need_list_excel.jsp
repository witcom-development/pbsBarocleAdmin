...
<body>
   <div style="height:200px; width:1200px;text-align:center; margin-top:10px;">
    ...
    <div>
        <table class="tb_condition">
            <tbody>
                ...
                <tr>
                    <td style="width:200px;" class="condition" ><label class="laType02">거치율</label></td>
                    <td style="width:400px;">
						<c:choose>
							<c:when test="${searchCondition.thresholdValue eq '선택'}">
								전체
							</c:when>
							<c:when test="${searchCondition.thresholdValue eq 'high'}">
								임계치 초과
							</c:when>
							<c:when test="${searchCondition.thresholdValue eq 'low'}">
								임계치 이하
							</c:when>
							<c:otherwise>
								-
    ...
    <table class="tb_type01" style="width : 1200px;">
        ...
        <tbody>
            <c:forEach var="result" items="${RelocationNeedList}" varStatus="loop">
                <tr>
                    <td class="title">${result.stationGrp}</td>
					...
					<td class="tx">${result.bikeTotCnt }</td>
                </tr>
            </c:forEach>
            <c:if test="${fn:length(RelocationNeedList) == 0 }">
                <tr><td colspan="6">
