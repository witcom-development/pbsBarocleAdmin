...
<%
    response.setHeader("Content-Disposition", "attachment; filename="+java.net.URLEncoder.encode("재고상세히스토리.xls", "UTF8"));
...
<body>
    ...
    <div>
        <table class="tb_condition">
           <tbody>
               <tr>
                   <td style="width:200px;" class="condition" ><label class="laType02">부품</label></td>
                   <td style="width:400px;">${inventData.partName}</td>
                   <td style="width:200px;" class="condition" ><label class="laType02">센터 명</label></td>
                   <td style="width:400px;">${inventData.centerName}</td>
                   <td style="width:200px;" class="condition" ><label class="laType02">수량</label></td>
                   <td style="width:400px;">${inventData.stockQunt}</td>
               </tr>
           </tbody>
        </table>
    </div>
        ...
        <tbody>
            <c:forEach var="history" items="${history}" varStatus="loop">
				<tr>
					...
					<td class="pl10" colspan="2">${history.modReasonDesc }</td>
					<td class="tc">${history.modDttm }</td>
				</tr>
			 </c:forEach>
             <c:if test="${fn:length(history) == 0}">
             <tr><td class="txCenter" colspan="5">no data</td></tr>
             </c:if>
        </tbody>
    </table>
</body>
