							...
							<tr> 
								...
								<td rowspan="2">
									<select class="select-category09" name="selectDay" id="selectDay">
                                       	...
                                       	<option value="2"<c:if test="${distributionAdminVo.dayCd eq '2'}">selected="selected"</c:if>>오후</option>
                                    </select>
                                </td>
                                ...
                                <td colspan="2"><input type="text" name="oil" style="width: 20px;"  value="${distributionAdminVo.oil}" />&nbsp;&nbsp;L</td>
                             </tr>
                             <tr> 
								<td>구 정보</td>
								<td class="tc">${distributionAdminVo.teamNm}</td>
                                <td>주행거리</td>
                                <td><input type="text" id="meter" name="meter" style="width: 40px;"  value="${distributionAdminVo.meter}" /></td>
                                <td>전&nbsp;&nbsp;<input type="text" name="firstMeter" style="width: 20px;"  value="${distributionAdminVo.firstMeter}" /></td>
                                <td>후&nbsp;&nbsp;<input type="text" id="lastMeter" name="lastMeter" style="width: 20px;"  value="${distributionAdminVo.lastMeter}" /></td>
                             </tr>
							</tbody>
							</table>	
							<br></br>
						...
						<table class="tb_type01" id="table_01">
							...
							<tbody>
								...
								<c:forEach var="list" items="${distributionList}"  varStatus="loop">
	                                ...
	                                <input type="hidden" name="arvlDttm" value="${list.odrDttm}" />
	                                <tr>
	                                    <td>${list.stationName }<input type="hidden" name="stationId" value="${list.stationId}" /></td>
	                                    <fmt:parseDate var="dateString" value="${list.odrDttm }" pattern="yyyy-MM-ddHH:mm:ss" />
	                                    <td><fmt:formatDate value="${dateString}" pattern="HH:mm:ss" />	</td>
	                                    <td><%-- <label id="bCnt_${lineCnt}"></label> --%><input type="text" id="bikeCount" name="bikeCount" style="width: 20px;" value="${list.arvlBikeCnt}" />
	                                    <div id="bLa_${lineCnt}" style="display: none">${list.stationId}_${list.odrDttm}</div>
	                                    </td>
										<td>${list.return_bike}<input type="hidden" name="returnBikeCnt" value="${list.return_bike}" /></td>
										<c:set var="returnBikeSum" value="${returnBikeSum + list.return_bike}"/>
	                                    <td>${list.rent_bike}<input type="hidden" name="rentBikeCnt" value="${list.rent_bike}" /></td>
	                                     <c:set var="rentBikeSum" value="${rentBikeSum + list.rent_bike}"/>	
	                                   	<td><input type="text" name="warehousing" style="width: 20px;" value="${list.wrhousngCnt}" /></td>
	                                   	<!-- <td>...</td> -->
	                                   	<td><input type="text" name="dlivy" style="width: 20px;"  value="${list.dlivyCnt}" /></td>
	                                   	<!-- <td>...</td> -->
	                                   	<td>
	                                   		<select class="select-category09" name="checkCode" id="checkCode">
                                                <option value="default">선택</option>
                                               <c:forEach var="cList" items="${codeList}"  varStatus="status">
                                               <c:if test="${list.chckCd eq cList.comCd}">
	                                               <option value="${cList.comCd}" selected="selected">${cList.comCdName}</option>
	                                           </c:if>
	                                           <c:if test="${list.chckCd ne cList.comCd}">
						 ...
						 <table class="tb_type02 mt20">
	                		...
	                		<tbody>
	                    		<tr>
	                        		...
	                        		<td><textarea class="detatextarea" id="etcDesc" name="etcDesc">${distributionAdminVo.etcDesc}</textarea>
	                        	</td>
	                        	</tr>
	                        	</tbody>
                        	</table>	
