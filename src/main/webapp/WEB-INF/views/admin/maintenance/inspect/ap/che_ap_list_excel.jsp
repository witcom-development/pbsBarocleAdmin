...
<%
    response.setHeader("Content-Disposition", "attachment; filename="+java.net.URLEncoder.encode("장애AP목록.xls", "UTF8"));
...
<body>
   <div style="height:200px; width:1200px;text-align:center; margin-top:10px;">
        ...
        <table class="tb_condition">
            <tbody>
                <tr>
                    ...
                    <td style="width:200px;" class="condition" ><label class="laType02">AP ID</label></td>
                    <td style="width:400px;">${searchCondition.apId}</td>
                </tr>
                <tr>
    ...
    <table class="tb_type01" style="width : 1200px;">
        ...
        <tbody>
            <c:forEach var="result" items="${InspectApList}" varStatus="loop">
                <tr>
                    <td class="title">${result.stationName}</td>
                    <td>${result.apId}</td>
                    <td>${result.bikeStatusName}</td>
					<td class="tx">${result.regDttm }</td>
                </tr>
            </c:forEach>
            <c:if test="${fn:length(InspectApList) == 0 }">
                <tr><td colspan="4">
