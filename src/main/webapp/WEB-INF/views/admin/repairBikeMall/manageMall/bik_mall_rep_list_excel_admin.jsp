    ...
    <table class="tb_type01" style="width : 100%;">
        <thead>
            <tr>
                ...
                <th>관리번호</th>
                <th>점포명</th>
                <th  class="case1">정비대수</br>기본교정</th>
          ...
          <c:forEach var="result" items="${ReportMallList}" varStatus="status">
			<tr>
				...
				<td>${status.count}</td>
                <td>${fn:substring(result.centerName,0,3) }</td>
                <td>${fn:substring(result.centerName,4,30) }</td>
                ...
                <td> <fmt:formatNumber pattern="#,###" value="${result.totCnt}"></fmt:formatNumber></td>  <!-- 정비건수</td> -->                                                                
