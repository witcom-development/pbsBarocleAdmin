...
<body>
	<div class="allwrap">
		...
		<div id="body">
			<div class="cont">
				...
				<div class="content">
					<div class="subject">
						<p class="pageTitle">외국관광객</p>
						<span>통계 &gt; 이용내역 &gt; <em>외국관광객</em></span>
					</div>
					<div class="page">
						<!-- 모바일 검색조건 S-->
						<div class="shBox_m">
						...
						<table class="tb_type01">
							...
							<c:forEach var="result" items="${dataList}" varStatus="status">
								<tr>
									...
									<td class="tc">${result.returnCnt}</td>
								</tr>
							</c:forEach>
							</tbody>
						</table>
    ...
    <script type="text/javascript">
   	$(function() {
   		var _this = foreignStatistics;
   		foreignStatistics.initCombo();
   		foreignStatistics.initBtn();
		...
		$("#stationGrpSlt").on('change', function(e){
   		...
   		$('#prevPage').on("click",function(){
			...
			foreignStatistics.pageSelect(currentPageNo-1);
		});
		...
		$('#movePage').on("click",function(){
			...
			if(!regExp.test(currentPageNo) ||(totalPageCount < currentPageNo)){
				...
				foreignStatistics.pageSelect(currentPageNo);
			}
		});
   	});
   	
   	var foreignStatistics = {};
   	foreignStatistics.stationGrp = '${searchCondition.stationGrp}';
   	foreignStatistics.stationId = '${searchCondition.stationId}';
   	
   	foreignStatistics.initCombo = function(){
    	var _this = foreignStatistics;
    ...
    foreignStatistics.exeStatisticsFnc = function(e) {
        ...
        $("#searchFrm").attr({method : 'post', action : '/use/foreignStatistics.do'}).submit();
        
        fn_loading(); 
        
    };
    
    foreignStatistics.exeStatisticsExcelFnc = function(e) {
        e.preventDefault();
        if(DateUtil.checkDateTerm( $( "[name='searchBgnDe']").val(),  $( "[name='searchEndDe']").val()) != 'ok') {
            alert('<spring:message code="fail.input.daterange"/>');
            return false;
        ...
        $("#searchFrm").attr({method : 'post', action : '/use/foreignStatisticsExcel.do'}).submit();

        
    };
    
   	foreignStatistics.pageSelect = function(pageNo) {
    	$("[name='currentPageNo']").val(pageNo);
    	$("#searchFrm").attr({action :'/use/foreignStatistics.do', method : 'post'}).submit();
    };
    </script>
</body>
</html>