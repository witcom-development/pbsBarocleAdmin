...
<body>
	<div class="allwrap">
		...
		<div id="body">
			<div class="cont">
				...
				<div class="content">
					<div class="subject">
						<p class="pageTitle">대여소 반납내역</p>
						<span>통계 &gt;  <em>대여소 반납내역</em></span>
					</div>
					<div class="page">
						<!-- 모바일 검색조건 S-->
						<div class="shBox_m">
    ...
    <script type="text/javascript">
   	$(function() {
   		var _this = returnStatistics;
   		returnStatistics.initCombo();
   		returnStatistics.initBtn();
		...
		$("#stationGrpSlt").on('change', function(e){
   		...
   		$('#prevPage').on("click",function(){
			...
			returnStatistics.pageSelect(currentPageNo-1);
		});
		...
		$('#movePage').on("click",function(){
			...
			if(!regExp.test(currentPageNo) ||(totalPageCount < currentPageNo)){
				...
				returnStatistics.pageSelect(currentPageNo);
			}
		});
   	});
   	
   	var returnStatistics = {};
   	returnStatistics.stationGrp = '${searchCondition.stationGrp}';
   	returnStatistics.stationId = '${searchCondition.stationId}';
   	
   	returnStatistics.initCombo = function(){
    	var _this = returnStatistics;
    ...
    returnStatistics.exeStatisticsFnc = function(e) {
        ...
        $("#searchFrm").attr({method : 'post', action : '/rent/returnStatistics.do'}).submit();
        
        fn_loading();
        
    };
    
    returnStatistics.exeStatisticsExcelFnc = function(e) {
        e.preventDefault();
        if(DateUtil.checkDateTerm( $( "[name='searchBgnDe']").val(),  $( "[name='searchEndDe']").val()) != 'ok') {
            alert('<spring:message code="fail.input.daterange"/>');
            return false;
        ...
        $("#searchFrm").attr({method : 'post', action : '/rent/returnStatisticsExcel.do'}).submit();
    };
    
   	returnStatistics.pageSelect = function(pageNo) {
    	$("[name='currentPageNo']").val(pageNo);
    	$("#searchFrm").attr({action :'/rent/returnStatistics.do', method : 'post'}).submit();
    };
    </script>
</body>
</html>