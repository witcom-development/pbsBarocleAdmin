	...
	<div class="allwrap">
		...
		<div id="body">
			<div class="cont">
				...
				<div class="content">
					<div class="subject">
						<p class="pageTitle">감면수단별 현황</p>
						<span>통계 &gt; 요금감면 &gt; <em>수단별 현황</em></span>
					</div>
					<form id="searchFrm">
									...
									<div class="shBoxSection">
										...
										<div class="section" style="margin-top: -25px; float: right;">
											<span style="position:absolute;">
												<!-- <input type="checkbox" value="Y" id="chkDailyTicket" name="chkDailyTicket" style="width: 20px; height: 20px;"/>
												<span style=" vertical-align: top; font-size:14px;">일일권만 조회</span> -->
											</span>
										</div>
								...
								<table class="tb_type03 tb_right">
									...
									<thead>
										<tr>
											...
											<th rowspan="2">총감면건</th>
											<th rowspan="2">총결제금액</th>
											<th colspan="4" style="color: #3232FF">감면 수단</th>
										</tr>
										<tr>
											<th style="color: #3232FF">청소년</th>
											<th style="color: #3232FF">국가 유공자</th>
											<th style="color: #3232FF">기초 생활자</th>
											<th style="color: #3232FF">한부모 가족</th>
									...
									<tbody>	
										<c:forEach var="result" items="${dataList}" varStatus="status">
			                                <tr>
			                                	...
			                                	<td style="text-align: right;"><fmt:formatNumber value="${result.rdt1Cnt}" pattern="#,###"/>&nbsp;&nbsp;</td>
			                                	<td style="text-align: right;"><fmt:formatNumber value="${result.rdt2Cnt}" pattern="#,###"/>&nbsp;&nbsp;</td>
			                                	<td style="text-align: right;"><fmt:formatNumber value="${result.rdt3Cnt}" pattern="#,###"/>&nbsp;&nbsp;</td>
			                                	<td style="text-align: right;"><fmt:formatNumber value="${result.rdt4Cnt}" pattern="#,###"/>&nbsp;&nbsp;</td>
			                                </tr>
			                            </c:forEach>    
    ...
    <script type="text/javascript">
    	$(function() {
	        ...
	        var chkDailyTicket = '${searchCondition.chkDailyTicket}';
	   		if(chkDailyTicket == 'Y'){
	   			$("#chkDailyTicket").attr("checked",true);
	   			$(".pageTitle").text("수단별 현황");
	   		} else{
	   			$(".pageTitle").text("수단별 현황");
	   		}
	   	...
	   	//next page
			$("#nextPage").click(function(){
				if($("#searchStartDate").val()  &&  $("#searchEndDate").val())
					...
					fn_loading();
					$("#searchFrm").attr({method : 'post', action : '/reduction/reductionTypeStatistics.do'}).submit();
			});
			...
			$('#excelBtn').on("click", function(){
	    		...
	    		$('#searchFrm').attr({method : 'post', action : '/reduction/reductionTypeStatisticsExcel.do'}).submit();
	   		}); 
