...
<body>
	<div class="allwrap">
							...
							<form id="frm" name="frm" accept-charset="utf-8">
								<fieldset>
									...
									<div class="shBoxSection">
										...
										<input type="hidden" name="usr_seq"></input>
									...
									<c:when test="${fn:length(smsList) > 0 }">
										<c:forEach items="${smsList}" var="smsList" varStatus="status">
											<tr>
												...
												<td>${smsList.gwCode}</td>
												<td>${smsList.attachNm}</td>
		...
		<form id="frmLink" name="frmLink" accept-charset="utf-8">
			<input type="hidden" name="usr_seq" id ="usr_seq"></input>
		</form>							
		...
		<script type="text/javascript">
			...
			$("input").on('focus',function(){
				...
				if(DateUtil.diffDays( sDate,  eDate, '-') >= 31){
		    		alert("조회기간은 31일 이내로 설정해주세요.");
		    		$(this).val(previous);
		    	} 
			});
			
			function msgAl(usrSeq){
			   	$("#usr_seq").val(usrSeq);
			    $("#frmLink").attr({action: '/admin/service/member/memberInfo.do', method :"post", target:"_blank"}).submit();
			}
			
			function checkSubmit(){ 
				$("[name='currentPageNo']").val(1);
				$("#frm").attr('action','/admin/service/policyMgmt/sendLongSmsList.do').submit();
				return true;
			}
			
			function linkPage(pageNo){
				$("[name='currentPageNo']").val(pageNo);
		        $("#frm").attr({method : "post", action : "<c:out value='/admin/service/policyMgmt/sendLongSmsList.do'/>"}).submit();
			}
