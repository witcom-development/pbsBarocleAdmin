...
<body>
			...
			<div class="cont">
				...
				<div class="content">
					...
					<div class="page">
						...
						<input type="hidden" name="partyUseCnt" value=""/>
						...
						<table class="tb_type01">
							...
							<tbody>
							<c:forEach var="result" items="${svccouponBuyList}" varStatus="status">
								<tr>
									<td>${result.mbName}</td>
								   <%--  <td>${result.couponName}</td> --%>
									<td class="title"><a href="#" onclick="fn_CouponDetail('${result.usrSeq}','${result.couponName}','${result.paymentClsCd}','${result.paymentDttm}','${result.partyUseCnt}'); return false;">${result.couponName}</a></td>
									<td class="tc">
										${result.paymentClsCdName}<c:if test='${result.partyUseCnt != 0}'>(${result.partyUseCnt}명)</c:if>
									</td>
									<td class="tc"><fmt:parseDate var="payDate" value="${result.paymentDttm}" pattern="yyyy-MM-dd"/>
									<fmt:formatDate pattern="yyyy-MM-dd" value="${payDate}"/> </td>
									<td class="tc">${result.couponSeqCnt}</td>
									<td class="tc">${result.couponRegisCnt}</td>
									<td class="tc">${result.couponUseCnt}</td>
									<td style="display:none" id="couponSeq"></td>
									
								</tr>
							</c:forEach>
							</tbody>
						</table>
						<c:if test='${fn:length(svccouponBuyList) == 0}'>
					...
					<c:choose>
						<c:when test="${fn:length(svccouponBuyList) > 0}">
							<div class="drpaging">
						</c:when>
						<c:otherwise>
    ...
    $(function() {
      ...
      //결제수단
        ...
        commonAjax.getCommonCode("BIL", function(data) {
        	 if(data !== null && data.codeList !== null) {
                 commCdBox.makeComboBox('CC', paymentMethodCd,data.codeList, "paymentClsCd");
             }
        });
    ...
    function fn_CouponDetail(usrSeq, couponName,paymentClsCd,paymentDttm,partyUseCnt){
    	$("input:hidden[name='usrSeq']").val(usrSeq);
    	$("input:hidden[name='couponName']").val(couponName);
    	$("input:hidden[name='paymentClsCd']").val(paymentClsCd);
    	$("input:hidden[name='paymentDttm']").val(paymentDttm);
    	$("input:hidden[name='partyUseCnt']").val(partyUseCnt);
    	//alert(usrSeq);
    	//alert(couponName);
    	//alert(paymentClsCd);
    	//alert(paymentDttm);
    	...
    	$("#frm").attr("action", "/getSvcCouponBuyDetail.do");
