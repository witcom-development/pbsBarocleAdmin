...
$(function() {
	...
	function fn_regist(){
		$("form[name=frm]").attr("action", "/admin/manage/station/stationGrpRemoteReturnRegist.do");
		$("form[name=frm]").submit();
	}
	
	function fn_Search(){
		$("[name='currentPageNo']").val(1);
		$("form[name=frm]").attr("action", "/admin/manage/station/stationGrpRemoteReturnList.do");
		$("form[name=frm]").submit();
	}
	...
	function fn_stationInfo(id, seq){
		$("#remote_return_seq").val(id);
		$("#station_grp_seq").val(seq);
		$("form[name=frm]").attr("action", "/admin/manage/station/stationGrpRemoteReturnInfo.do");
		$("form[name=frm]").submit();
	}
								...
								<tbody>
									<c:forEach items="${stationList }"	var="list">
										<tr>
											...
											<td class="title" onclick="fn_stationInfo('${list.remote_return_seq}', '${list.station_grp_seq}')"><a href="#">
...
<%-- 												<c:when test="${list.remote_use_yn eq 'C'}">폐쇄</c:when> --%>
<%-- 												<c:when test="${list.remote_use_yn eq 'T'}">임시 폐쇄</c:when> --%>
<%-- 												<c:when test="${list.remote_use_yn eq 'S'}">임시 폐쇄(예정)</c:when> --%>
						...
						<input type="hidden" name="station_grp_seq" id="station_grp_seq">
						<input type="hidden" name="remote_return_seq" id="remote_return_seq">
				</div>
				<!--conteent E-->			
				</form>
				<!--content E-->
