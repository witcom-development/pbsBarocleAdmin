...
<!-- 
	...
	내용     : 정비직원 근태 조회 화면
	TODO : 
	ASIS : 2020-10-21 : 승인처리 시 변경사항없다면 알림처리 와 선택값이 없을 경우도 알림처리 
	
		   fmt:parsNumber 를 이용해 근무일이 당일 이후라면 승인처리 불가능하게 처리.
...
<!-- solbit test-테스트 페이지  S   -->
		                        	...
		                        	<table class="tb_type02 my-3">
		                        		...
		                        		<thead>
		                        			<tr>
                        						...
                        						<th class="top text-center">근무시간</th>
                        						<th class="top text-center" colspan="2">정비 외 시간</th>
			                        		...
			                        		<c:forEach items="${workList}" var="list" varStatus="vst">
			                        			...
			                        			<tr>
				                        			...
				                        			<c:if test="${list.dtlCd ne 'WRK_001'}">
				                        				<td class="text-center"><c:out value="${list.dtlName}"></c:out></td>
				                        			</c:if>
				                        			<c:if test="${list.dtlCd eq 'WRK_001'}">
				                        				<td class="text-center"><c:out value="${list.etc}"></c:out></td>
				                        			</c:if>
    ...
    $(function(){
		...
		$("#approvedBtn").on('click',function(e){//승인저장 버튼 : 맨처음 받아온 aprroved 값과 화면에서 처리한 aprroved 값들을 비교해 바뀐 것만 처리
			...
			if(repairWorkInfo.chgApdArr != null){
				...
				if(checkChange){
					...
					$.ajax({
						type : "post",
						url : "/updateRepairWorkAprvdAjax.do",
						traditional : true,
						data : params,
		...
		$("#searchBtn").on("click",function(e){
			...
			$("#searchFrm").attr({method : 'post', action :'/getRepairWork.do'}).submit();
    ...
    /*********** 변수 & 함수 S ***********/
   	...
   	repairWorkInfo.getCenterCode = function(pCb){
