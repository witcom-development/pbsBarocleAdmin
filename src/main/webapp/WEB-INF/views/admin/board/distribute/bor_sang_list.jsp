			...
			<div class="cont">
				...
				<div class="content">
									...
									<c:when test="${fn:length(distBoardList) > 0 }">
										<c:forEach items="${distBoardList}" var="distBoardList" varStatus="status">
											<tr>
												...
												<td class="title"><a href="/admin/board/distribute/sangBoardEdit.do?mode=edit&distSeq=<c:out value='${distBoardList.distSeq}'/><c:if test="${distributeBoardView.currentPageNo!=0}">&currentPageNo=<c:out value='${distributeBoardView.currentPageNo}'/></c:if><c:out value='${parameterInfo}' escapeXml='false'/>"><c:out value="${distBoardList.distTitle}"/></a></td>
					...
					<c:choose>
						...
						<div class="btnArea">
							<span class="right"><button class="btnType01 bc_green" onclick="location.href='/admin/board/distribute/sangBoardEdit.do<c:if test="${distributeBoardView.currentPageNo ne '' && distributeBoardView.currentPageNo ne null}">?currentPageNo=<c:out value='${distributeBoardView.currentPageNo}'/></c:if><c:out value='${parameterInfo}' escapeXml='false'/>';">등록</button></span>
						</div>
					</div>
				</div>
				<!--content E-->
		...
		<script type="text/javascript">
			...
			function checkSubmit(){
				if($("#sDate").val()!=''&&$("#eDate").val()!=''&&$("#sDate").val()>$("#eDate").val()){
					...
					return false;
				} else { $("#frm").attr('action','/admin/board/distribute/getSangBoardList.do').submit(); }
				return true;
			}
			var previousValue;
